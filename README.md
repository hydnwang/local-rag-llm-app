...
**Ingestion path:** uploaded files are handed to a Dagster job that parses (PDF/Markdown/text), chunks, embeds (`all-MiniLM-L6-v2`), and writes vectors into Qdrant. FastAPI polls Dagster for run completion before confirming success to the UI.
...
│
├── api/
│   ├── main.py               # FastAPI app: /query (SSE), /ingest, /health; Prometheus /metrics
...
│   ├── ingest.py               # shared ingestion helpers
...
Grafana dashboards are currently configured manually through its UI rather than provisioned from files in this repo.
...
- **No true query cancellation or ingestion abort yet.** There's currently no way to stop an in-flight query or a running ingestion job partway through — you can block a *new* question from starting while one is in progress (see above), but not stop the one that's already running. Adding real cancellation requires converting the query graph's nodes to async so an in-flight Ollama call actually gets interrupted, not just the UI's connection to it; ingestion abort would call Dagster's run-termination API. Both are planned for the next phase.
- **No way to delete an ingested file's vectors from Qdrant yet.** Ingested documents accumulate; there's currently no UI or API path to remove one. Planned for the next phase, alongside cancellation — both raise a related question (how to reliably reference a specific ingested file from the UI) worth solving together.
